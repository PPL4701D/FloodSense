...
import { useReportEdit } from '@/lib/hooks/useReportEdit';
...
export default function ReportEditPage() {
  ...
  useEffect(() => {
    if (!authLoading && !isAuthenticated) {
      router.push(`/login?redirect=/report/${id}/edit`);
    }
  ...
  useEffect(() => {
    ...
    const fetchReport = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('reports')
          .select('*, report_photos(id, storage_path)')
          .eq('id', id)
        ...
        let lat = 0, lng = 0;
        if (data.location) {
          const wktMatch = data.location.match(/POINT\(([^ ]+) ([^)]+)\)/);
          if (wktMatch) {
            lng = parseFloat(wktMatch[1]);
            lat = parseFloat(wktMatch[2]);
          } else if (/^[0-9A-Fa-f]+$/.test(data.location)) {
            // Very simplified parsing for WKB, assuming it's point data
            const offset = data.location.length >= 50 && data.location.slice(8, 10) === '20' ? 9 : 5;
        ...
        setValue('water_height_cm', data.water_height_cm || null);
        setValue('is_surge_receding', data.is_surge_receding);
        setValue('address', data.address || '');

        if (data.report_photos) {
          const photos = data.report_photos.map((p: any) => ({
            id: p.id,
            url: supabase.storage.from('flood-photos').getPublicUrl(p.storage_path).data.publicUrl
          }));
    ...
    fetchReport();
  }, [id, user, setValue, router, supabase]);
  ...
  const handlePhotoAdd = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (existingPhotos.length + photos.length + files.length > 3) {
      alert('Maksimal 3 foto total (termasuk foto yang sudah ada)');
      return;
    }
    ...
    return (
      <div style={{ minHeight: '100dvh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', background: 'var(--bg-primary)' }}>
        <div className="animate-fade-in" style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '1.25rem', fontWeight: 700, marginBottom: '0.5rem' }}>Laporan Diperbarui!</h2>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>Perubahan laporan Anda telah disimpan.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ height: '100dvh', background: 'var(--bg-primary)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div className="glass" style={{
        ...
        <div style={{ flex: 1 }}>
          <h1 style={{ fontSize: '1rem', fontWeight: 700 }}>Edit Laporan</h1>
          ...
          <div style={{ padding: '1rem 1rem 2rem', maxWidth: '480px', margin: '0 auto' }}>
            ...
            {step === 1 && (
              <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                <div style={{ textAlign: 'center', padding: '1rem 0' }}>
                  ...
                  <h2 style={{ fontSize: '1.125rem', fontWeight: 700, marginBottom: '0.375rem' }}>Edit Lokasi</h2>
                  <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>Ubah lokasi jika tidak akurat</p>
                </div>
                ...
                {isLocating ? (
                  ...
                  <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', overflow: 'visible' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      ...
                      <div>
                        <p style={{ fontWeight: 600, fontSize: '0.875rem' }}>Lokasi Terpilih</p>
            ...
            {step === 3 && (
              <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                <div style={{ textAlign: 'center', padding: '1rem 0' }}>
                  ...
                  <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>Tambahkan foto baru (maks total 3)</p>
                </div>

                {existingPhotos.length > 0 && (
                  <div>
                    <p style={{ fontSize: '0.8125rem', fontWeight: 500, marginBottom: '0.5rem', color: 'var(--text-secondary)' }}>Foto yang sudah ada:</p>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem', marginBottom: '1rem' }}>
                      {existingPhotos.map((photo, i) => (
                        <div key={photo.id} style={{ position: 'relative', aspectRatio: '1', borderRadius: 'var(--radius-md)', overflow: 'hidden', border: '1px solid var(--border-primary)' }}>
                          <img src={photo.url} alt={`Foto ada ${i + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  <button type="button" onClick={() => setStep(4)} className="btn btn-primary" style={{ width: '100%' }}>Lanjut</button>
                </div>
              </div>
            )}
            ...
            {step === 4 && (
              <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <h2 style={{ fontSize: '1.125rem', fontWeight: 700 }}>Tinjau Perubahan Laporan</h2>
                  ...
                  {(existingPhotos.length + photos.length) > 0 && (
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem' }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: '0.8125rem' }}>{existingPhotos.length + photos.length} foto terlampir</p>
                      </div>
                    </div>
                  )}
                </div>
